...
public class OvrAvatarSkinnedMeshRenderComponent : OvrAvatarRenderComponent
{
    ...
    internal void CreateCollider(Transform transform)
    {
            ...
            transform.name.Contains("hands"))
        {
                ...
                transform.name.Contains("pinky"))
            {
                if (!transform.name.EndsWith("0"))
                {
                    ...
                    if (!transform.name.EndsWith("1"))
                    {
                        ...
                        collider.center = Phalanges.GetCenter(transform.name.Contains("_1_"));
                    ...
                    else
                    {
                        ...
                        collider.center = Metacarpals.GetCenter(transform.name.Contains("_1_"));
                        collider.direction = 0;
                    }
                }
            }
            else if (transform.name.Contains("grip"))
            {
                ...
                collider.center = new Vector3(
                    ((transform.name.Contains("_1_")) ? -1 : 1) * 0.01f, 0.01f, 0.02f);
            }
        }
